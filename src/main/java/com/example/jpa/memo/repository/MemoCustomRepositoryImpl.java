...
//        "from Memo m left join m.member x where m.text like %:text% "
        ...
        //3. countQuery 실행
        //"select count(m) from Memo m left join m.member x where m.text like %:text%"
        long total = jpaQueryFactory
                .select(memo)
                .from(memo)
                .leftJoin(memo.member, member)
                .where(builder) //불린빌더
                .fetch().size(); //카운트 쿼리 구함
        
        //4. Page객체에 맵핑
        PageImpl<MemberMemoDTO> page = new PageImpl<>(list, pageable, total); //데이터, 페이지객체, 전체게시글 수

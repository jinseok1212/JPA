...
public class JPABasicTest01 {
    ...
    public void testCode01() {
         for(int i = 0; i <= 100; i++)
         memoRepository.save(Memo.builder().writer("example" + i).text("sample" + i).build());
    }
    //select - find가 뭍은 메서드를 사용
    @Test
    ...
    public void testCode04() {
        //내부적으로 select을 통해 번호를 확인하고 없으면 insert 있으면 update를 처리함
        Memo result = memoRepository.save(Memo.builder().mno(50L).writer("업데이트완").text("update").build());

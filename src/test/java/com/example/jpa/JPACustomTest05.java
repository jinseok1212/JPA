...
//        int result = memoRepository.updateTest("구현체를 통한 update", 15L);
...
//        for(int i = 1; i <= 5; i++) {
//            memberRepository.save(Member.builder().id("abc" + i).name("admin" + 1).build());//sign_date는 JPA대신에서 데이터를 삽입
//
//        }
//    }
    ...
    //eager조인 vs lazy조인
    @Test
    @Transactional //Lazy을 쓰려면 트랜잭션으로 묶어줘야함
    ...
    public void testCode11() {
        Page<MemberMemoDTO> list = memoRepository.joinPage("1", PageRequest.of(0, 10));
